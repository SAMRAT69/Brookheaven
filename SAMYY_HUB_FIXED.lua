...
local Window = Libary:MakeWindow({
    Title = "SAMYY HUB | Brookhaven RP ",
    Icon = "https://files.catbox.moe/zwl1xo.webp"
    SubTitle = "by: SAMRAT",
    LoadText = "Carregando SAMYY HUB",
    Flags = "SAMYY_HUB_Brookhaven"
})
...
InfoTab:AddParagraph({ "Owner / Developer: SAMRAT", "SAMRAT" })
InfoTab:AddParagraph({ "Collaborators:", "SAMRAT" })
InfoTab:AddParagraph({ "You are using: SAMYY HUB Brookhaven", "SAMYY HUB Brookhaven " })
...
TrollTab:AddButton({
    Name = "Black Hole",
    Description = " Activating this pulls parts toward your character",
    Callback = function()
...
Label.Text = "Bring Parts | Made by: SAMRAT
...
TrollTab:AddToggle({
    Name = "RGB Hair",
    Description = "Deixa Seu RGB Hair",
    Default = false,
    Callback = function(value)
        isActive = value
        if isActive then
...
local function BringWithCouch()
    ...
    if not couch then
        warn("Error: Couch não encontrado no Backpack")
        return
    end
    ...
    Troll:AddButton({
        ...
        Callback = function()
            ...
            if killDropdown and #tablePlayers > 0 then
                ...
                print("Error: Dropdown não encontrado ou nenhum jogador disponível.")
            end
        end
    })
    ...
    Troll:AddButton({
        ...
        Callback = function()
            if not selectedPlayerName or not Players:FindFirstChild(selectedPlayerName) then
                print("Error: Player não selecionado ou não existe")
                return
            end
            local character = LocalPlayer.Character
            ...
            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                print("Error: Player alvo não encontrado ou sem HumanoidRootPart")
            end
        end
    })
...
Troll:AddButton({
    ...
    Callback = function()
        if not selectedPlayerName or not game.Players:FindFirstChild(selectedPlayerName) then
            warn("No player selected ou não existe")
            return
        end
    ...
    HouseTab:AddButton({
        ...
        Callback = function()
            ...
            if House and game.Players.LocalPlayer.Character then
                ...
                print("House não encontrada: " .. tostring(SelectHouse))
            end
        end
    })
end)
...
pcall(function()
    NoclipDoor = HouseTab:AddToggle({
        Name = "Atravessar Porta da House",
        Description = "",
        Default = false,
        Callback = function(Value)
            pcall(function()
...
-- Shutdown Server (Internet Errorr) Button
Tab:AddButton({
    Name = "Shutdown Servidor (Error de Internet)",
    Callback = function()
        for m = 1, 535 do
            local args = {
                [1] = "PickingTools",
...
-- Shutdown Server (Timeout Errorr) Button
Tab:AddButton({
    Name = "Shutdown Servidor (Error de Conexão Expirada)",
    Callback = function()
        for m = 1, 635 do
            local args = {
                [1] = "PickingTools",
...
ChildTab:AddButton({
    ...
    Callback = function()
        ...
        if not workspace:FindFirstChild(pl.Name) or not workspace[pl.Name]:FindFirstChild("FollowCharacter") then
            ...
            if not success then
                warn("Error ao enviar criança: " .. err)
            end
        end
...
ChildTab:AddButton({
    ...
    Callback = function()
        ...
        if not success then
            warn("Error ao retornar criança: " .. err)
        end
        ...
        if not success then
            warn("Error ao spawnar criança: " .. err)
        end
    end
})
...
ChildTab:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            if not chasingplayer then
                warn("No player selected para spectar!")
                return false
            end
...
LocalPlayerTab:AddToggle({
    ...
    Callback = function(bool)
        ...
        if not selectedHeadSit or selectedHeadSit == "" then
            warn("No player selected para Head Sit!")
            return false
        end
        ...
        if bool then
            if selectedPlayer and selectedPlayer.Character then
                ...
                warn("Selected player não encontrado ou sem Character!")
                return false
            end
        else
            if headSitConnection then
...
local teleportButtons = {
    ...
    {"À House Deserta", CFrame.new(986, 4, 63)},
